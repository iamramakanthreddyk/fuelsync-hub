...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    console.log(`[ADMIN-AUTH] Login attempt: ${JSON.stringify({
      requestId,
    ...
    console.log(`[ADMIN-AUTH] Login successful: ${JSON.stringify({
      requestId,
      email,
      timestamp: new Date().toISOString()
    })}`);
    ...
    console.error(`[ADMIN-AUTH] Login error: ${JSON.stringify({
      requestId,
      error: error.message,
      stack: error.stack,
      email: req.body.email,
...
export const logout = async (req: Request, res: Response) => {
  try {
    ...
    // Logout admin
    await adminAuthService.logoutAdmin(req.admin.id, token);
